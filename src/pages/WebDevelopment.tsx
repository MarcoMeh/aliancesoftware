        ...
        <section className="py-16">
          <div className="container mx-auto px-6">
            ...
            <div className="space-y-24"> {/* Increased space between projects */}
              {websiteData.map((project, index) => (
                <div
                  key={project.id}
                  className="bg-[#0C1530]/70 backdrop-blur-md border border-[#1e3a8a]/50 rounded-xl
                             p-8 md:p-12 text-center relative overflow-hidden group"
                >
                  <h3 className="text-4xl font-bold text-white mb-4 group-hover:text-[#60a5fa] transition-colors duration-300">
                    {project.name}
                  </h3>
                  ...
                  <div className="relative w-full h-[400px] mb-8"> {/* Fixed height for the slider */}
                    <Swiper
                      pagination={{
                        clickable: true,
                        dynamicBullets: true,
                      ...
                      navigation={{
                        nextEl: `.swiper-button-next-${project.id}`, // Unique class for each project's navigation
                        prevEl: `.swiper-button-prev-${project.id}`,
                      }}
                      modules={[Pagination, SwiperNavigation, Autoplay]}
                      className="myHorizontalSwiper h-full w-full rounded-xl shadow-lg"
                      autoplay={{
                        delay: 3000,
                      ...
                      loop={true}
                    >
                      {/* Check if project.images exists and is an array */}
                      {project.images && project.images.map((image, imgIndex) => (
                        <SwiperSlide key={imgIndex}>
                          <img
                            src={image}
                            alt={`${project.name} - Screenshot ${imgIndex + 1}`}
                            className="w-full h-full object-contain rounded-xl" // Use object-contain to fit images without cropping
                          />
                        </SwiperSlide>
                      ))}
                    </Swiper>

                    {/* Custom Horizontal Navigation Arrows for THIS slider */}
                    <div className={`swiper-button-prev-${project.id} absolute top-1/2 -translate-y-1/2 left-4 z-10 cursor-pointer p-3 rounded-full bg-blue-700/50 hover:bg-blue-700 transition-colors`}>
                        <ArrowLeft className="w-6 h-6 text-white" />
                    </div>
                    <div className={`swiper-button-next-${project.id} absolute top-1/2 -translate-y-1/2 right-4 z-10 cursor-pointer p-3 rounded-full bg-blue-700/50 hover:bg-blue-700 transition-colors`}>
